...
# Resolve the launcher activity from the installed package instead of hardcoding it. The Java
# Callable Wrapper package name is generated by the Android toolchain and is not stable: the Mono
# build emits `crc64<hash>` while the NativeAOT typemap emits `scrc64<hash>`, and the hash itself
# changes whenever the activity's namespace or assembly changes. Hardcoding it silently breaks the
# run with "Activity class {...} does not exist" and an empty test result.
UNO_UITEST_ACTIVITY=$($ANDROID_HOME/platform-tools/adb shell cmd package resolve-activity --brief "$UNO_UITEST_APP_ID" 2>/dev/null | tail -n 1 | tr -d '\r' || true)

if [[ "$UNO_UITEST_ACTIVITY" != */* ]]; then
	echo "ERROR: could not resolve a launcher activity for $UNO_UITEST_APP_ID"
	$ANDROID_HOME/platform-tools/adb shell cmd package resolve-activity "$UNO_UITEST_APP_ID" || true
	exit 1
fi

echo "Resolved launcher activity: $UNO_UITEST_ACTIVITY"

# start the android app using environment variables using adb
$ANDROID_HOME/platform-tools/adb shell am start \
  -n "$UNO_UITEST_ACTIVITY" \
